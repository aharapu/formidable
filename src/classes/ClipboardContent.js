...
export class ClipboardContent {
    ...
    addTestScenarios(scenarios = []) {
        ...
        scenarios.forEach((scenario) => {
            this.content += `<li>${scenario.scenarioName}</li>`; // TODO -> rename scenarioName to name
            ...
            scenario.when.forEach((w, idx) => {
                const prefix = idx === 0 ? 'When' : 'And';
                const rowContent = `<strong>${prefix} </strong>` + w.value;
                this._private_addHtmlTagString({ tag: 'li', content: rowContent });
            });
            scenario.then.forEach((t, idx) => {
                const prefix = idx === 0 ? 'Then' : 'And';
                const rowContent = `<strong>${prefix} </strong>` + t.value;
                this._private_addHtmlTagString({ tag: 'li', content: rowContent });
            });
            this.content += '</ul>';
        });
